...
- ### [🔥 기술 챌린지](#🔥-기술-챌린지)
  - [1. ⏱️ 10분 단위의 시간 셀을 어떻게 렌더링 할 수 있을까?](#1-10분-단위의-시간-셀을-어떻게-렌더링-할-수-있을까?)
    - [1.1 아이디어 : 1시간 단위로 24번 렌더링 하기](#11-아이디어--1시간-단위로-24번-렌더링-하기)
    - [1.2 문제 인식 : 반복되는 이벤트 핸들러 전달로 인한 가독성 저하](#12-문제-인식--반복되는-이벤트-핸들러-전달로-인한-가독성-저하)
...
개인 프로젝트 주제로는 제가 활용성 있게 사용했던 시중에 발매되어 있는 PDS 다이어리를 웹으로 구현하고자 했습니다. 이번 프로젝트를 통해서 달성하고자 했던 로는 목표는 크게 2가지가 있었습니다.

<br />

> 1. 웹에서 다이어리 작성시 편리한 UI/UX 제공
>
> 2. 텍스트 편집기 & 드래그 기능을 직접 구현을 통한 원리 이해


### 1. 웹에서 다이어리 작성시 편리한 UI/UX 제공
- 사용자가 일정관리를 수기로 작성하지 않고 웹에서 사용하고자 할때는 어떤 장점이 있어야 할까 사용자 경험 측면에서 생각해 보았습니다.<br />
...
#### 1.1 아이디어 : 1시간 단위로 24번 렌더링 하기
> 1시간을 6칸으로 나눠서 세로로 24번 반복 렌더링 하기
...
#### 1.2 문제 인식 : 반복되는 이벤트 핸들러 전달로 인한 가독성 저하
...
> 24시간을 10분으로 나눈 144개의 인덱스를 반복 렌더링 하기
...
#### 4.1 아이디어 : `execCommand()`를 사용해서 스크롤 된 텍스트에 스타일 적용하기
